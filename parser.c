...
int leer_orden(orden* O, FILE* stream) {
  ...
  if (O == NULL || O->orden_cruda != NULL || O->buffer_trozos != NULL ||
      O->argv != NULL)
  ...
  O->buffer_trozos = (char*)malloc((1 + strlen(p)) * sizeof(char));

  if (O->buffer_trozos == NULL) return ERROR_FALTA_MEMORIA;
  ...
  if (argv == NULL) return ERROR_FALTA_MEMORIA;
      ...
      switch (redireccion)  // donde corresponda
      {
        ...
        case 0:  // Argumento ordinario -> argv
          ...
          {                      // más espacio en argv
            ...
            argv = (char**)realloc(O->argv = argv, argr * sizeof(char**));
...
static int leer_linea(char** plinea, FILE* stream) {
  ...
  if (plinea == NULL || *plinea != NULL) return ERROR_PARAMETROS_MAL;
  ...
  *plinea = p = (char*)malloc(tam * sizeof(char));
  ...
  if (*plinea == NULL) return ERROR_FALTA_MEMORIA;
  ...
  for (;;)  // Bucle infinito
  {
    ...
    {                // array... (duplicar tamaño)
      tam *= 2;
      p = tam <= 0 ? NULL : (char*)realloc(*plinea, tam * sizeof(char));
